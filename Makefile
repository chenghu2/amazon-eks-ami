...
ifeq ($(arch), arm64)
	...
	ami_name ?= cflt-eks-arm64-node-$(K8S_VERSION_MINOR)-v$(shell date +'%Y%m%d')$(if $(enable_fips_mode),-fips)
else
	instance_type ?= m5.large
	ami_name ?= cflt-eks-node-$(K8S_VERSION_MINOR)-v$(shell date +'%Y%m%d')$(if $(enable_fips_mode),-fips)
endif
